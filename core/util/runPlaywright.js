...
async function checkBrowserVersion (page) {
...
async function setUpConsoleListener (readyEvent, page) {
...
async function handleBeforeScript (scenario, config, engineScriptsPath, page, viewport, isReference, browser) {
...
async function openUrl(scenario, page, isReference) {
...
async function processScenarioView (scenario, variantOrScenarioLabelSafe, scenarioLabelSafe, viewport, config) {
  ...
  const playwrightArgs = Object.assign(
    {},
    {
      ignoreHTTPSErrors: true, /* TODO; find how to ignore HTTPS error */
      headless: !config.debugWindow /* TODO; find headless option */
    },
    config.engineOptions
  );

  const browser = await chromium.launch(playwrightArgs); /* TODO; would be good to create browser instance only one */
  ...
  const readyPromise = await setUpConsoleListener(readyEvent, page);

  await checkBrowserVersion(page);

  let result;
  const commands = async () => {
    await handleBeforeScript(scenario, config, engineScriptsPath, page, viewport, isReference, context);
    await openUrl(scenario, page, isReference);
